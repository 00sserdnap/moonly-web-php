/* =============================================
   MOONLY — PLANES PYTHON (python.js)
   ============================================= */
document.addEventListener("DOMContentLoaded", function () {
    ...
    var PLANS = [
        { name: "Script",  priceUsd: 1.50, cpu: "50%",  ram: "512MB", storage: "5GB"  },
        { name: "App",     priceUsd: 3.00, cpu: "75%",  ram: "1GB",   storage: "15GB", popular: true },
        { name: "Service", priceUsd: 6.00, cpu: "150%", ram: "2GB",   storage: "30GB" },
        { name: "Cluster",  priceUsd: 12.00, cpu: "300%", ram: "4GB", storage: "60GB" }
    ];

    var grid = document.getElementById("python-plans-grid");
    ...
    function renderCards() {
        ...
        PLANS.forEach(function (plan) {
            ...
            if (plan.popular) {
                card.style.borderColor = "#3776ab";
                card.style.boxShadow = "0 0 0 1px #3776ab, 0 16px 36px rgba(55,118,171,.12)";
            }

            var popularTag = plan.popular
                ? '<span class="plan-popular-tag" style="background: #3776ab; color: #fff; box-shadow: 0 6px 16px rgba(55,118,171,.4);">' + t("plans.popular_tag") + '</span>'
                : "";
            ...
            card.innerHTML =
                popularTag +
                '<div class="plan-name" style="color: #3776ab; font-size: 1.1rem;">' + plan.name + '</div>' +
                '<div class="plan-price-row" style="margin-top: 15px;">' +
                '<span class="plan-price" style="color:#3776ab;" data-usd="' + totalUsd.toFixed(2) + '">' + formatUsdInline(totalUsd) + '</span>' +
                discountBadgeHtml +
                '</div>' +
                ...
                '<div class="plan-spec"><i class="fa-solid fa-microchip" style="color:#3776ab;"></i> <span>' + plan.cpu + ' CPU</span></div>' +
                '<div class="plan-spec"><i class="fa-solid fa-memory" style="color:#3776ab;"></i> <span>' + plan.ram + ' RAM</span></div>' +
                '<div class="plan-spec"><i class="fa-solid fa-hard-drive" style="color:#3776ab;"></i> <span>' + plan.storage + ' SSD</span></div>' +
                '<div class="plan-spec"><i class="fa-solid fa-rotate" style="color:#3776ab;"></i> <span>' + t("python.feat_autorestart") + '</span></div>' +
                '</div>' +

                '<a href="https://billing.moonly.es" target="_blank" class="plan-btn" style="background: #3776ab; border-color: #3776ab; color: #fff;">' +
